...
export default function MasterDashboard({ userProfile, addToast }: MasterDashboardProps) {
  // Navigation
  // 'stats' | 'courses' | 'codes' | 'students' | 'requests' | 'announcements' | 'settings'
  const [tab, setTab] = useState<'stats' | 'courses' | 'codes' | 'students' | 'requests' | 'announcements' | 'settings'>('stats');

  // Master Credentials and Account Settings
  const [masterAuthEmail, setMasterAuthEmail] = useState('master@mr-abdallah.com');
  const [masterAuthPassword, setMasterAuthPassword] = useState('2026');
  const [masterAuthPin, setMasterAuthPin] = useState('2026');
  const [masterAuthUpdatedAt, setMasterAuthUpdatedAt] = useState<string | null>(null);
  const [isSavingMasterAuth, setIsSavingMasterAuth] = useState(false);
  const [showMasterAuthPass, setShowMasterAuthPass] = useState(false);
  ...
  // Edit Student Subscription Date Form & Parent Tracking Notes
  const [editingStudent, setEditingStudent] = useState<UserProfile | null>(null);
  const [newSubExpiry, setNewSubExpiry] = useState('');
  const [studentTeacherNotes, setStudentTeacherNotes] = useState('');
  const [studentBehaviorRating, setStudentBehaviorRating] = useState<'excellent' | 'very_good' | 'good' | 'needs_followup'>('very_good');
  ...
  useEffect(() => {
    ...
    fetchMasterAuthSettings();
  }, []);

  const fetchMasterAuthSettings = async () => {
    try {
      const snap = await getDoc(doc(db, 'settings', 'master_auth'));
      if (snap.exists()) {
        const data = snap.data();
        if (data.email) setMasterAuthEmail(data.email);
        if (data.password) setMasterAuthPassword(data.password);
        if (data.pin) setMasterAuthPin(data.pin);
        if (data.updatedAt) setMasterAuthUpdatedAt(data.updatedAt);
      }
    } catch (err) {
      console.error('Error fetching master auth settings:', err);
    }
  };

  const handleSaveMasterAuth = async (e: React.FormEvent) => {
    e.preventDefault();
    const cleanEmail = masterAuthEmail.trim().toLowerCase();
    const cleanPass = masterAuthPassword.trim();
    const cleanPin = masterAuthPin.trim();

    if (!cleanEmail || !cleanPass) {
      addToast('يرجى إدخال البريد الإلكتروني وكلمة المرور للمستر.', 'error');
      return;
    }

    setIsSavingMasterAuth(true);
    try {
      const nowIso = new Date().toISOString();
      await setDoc(doc(db, 'settings', 'master_auth'), {
        email: cleanEmail,
        password: cleanPass,
        pin: cleanPin || '2026',
        updatedAt: nowIso,
      ...
      setMasterAuthUpdatedAt(nowIso);
      localStorage.setItem('master_saved_email', cleanEmail);
      addToast('تم تحديث وحفظ بيانات دخول المستر بنجاح! 🔒', 'success');
    } catch (err) {
      console.error('Error saving master auth:', err);
      addToast('حدث خطأ أثناء حفظ بيانات المستر، يرجى المحاولة لاحقاً.', 'error');
    } finally {
      setIsSavingMasterAuth(false);
    }
  };
  ...
  // Edit Student subscription & parent followup notes directly
  const handleUpdateStudentSub = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'users', editingStudent.uid), {
        ...
        teacherNotes: studentTeacherNotes,
        behaviorRating: studentBehaviorRating,
        parentPhone: studentParentPhone,
      });

      addToast(`تم تحديث بيانات وملاحظات الطالب ${editingStudent.name} لولي الأمر بنجاح!`, 'success');
      ...
      console.error(err);
      addToast('فشل تعديل بيانات الطالب.', 'error');
    }
  };

  // Open & Send Parent WhatsApp Followup Report
  const handleOpenParentReport = async (student: UserProfile) => {
    try {
      addToast('جاري استخراج تقرير ولي الأمر من قاعدة البيانات...', 'info');
      const report = await searchStudentForParent(student.uid);
      if (!report) {
        addToast('تعذر استخراج تقرير الطالب من قاعدة البيانات.', 'error');
        return;
      }
      const text = generateParentWhatsAppText(report);
      ...
      const url = cleanPhone
        ...
        : `https://wa.me/201102140676?text=${encodeURIComponent(text)}`;
      window.open(url, '_blank', 'noreferrer');
      addToast('تم تجهيز التقرير وفتح محادثة واتساب ولي الأمر بنجاح! 🚀', 'success');
    } catch (e) {
      console.error(e);
      addToast('حدث خطأ أثناء إعداد التقرير.', 'error');
    }
  };
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen font-sans" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                              ...
                              <td className="p-4 text-center">
                               ...
                               <td className="p-4 text-center">
                                 <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                   ...
                                   <button
                                     onClick={() => handleOpenParentReport(s)}
                                     className="flex items-center gap-1 px-2.5 py-1.5 bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100 text-emerald-700 dark:text-emerald-300 rounded-lg text-xs font-bold transition-all cursor-pointer"
                                     title="إرسال تقرير المتابعة لولي الأمر عبر واتساب"
                                   >
                                   ...
                                   <button
                                     onClick={() => {
                                       ...
                                       setStudentTeacherNotes(s.teacherNotes || '');
                                       setStudentBehaviorRating(s.behaviorRating || 'very_good');
               ...
               <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                 <div className="bg-white dark:bg-slate-900 rounded-2xl border p-6 w-full max-w-md space-y-4">
                   ...
                   <form onSubmit={handleUpdateStudentSub} className="space-y-4 text-right">
                     <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/40 rounded-xl p-3 flex items-center justify-between">
                       <div>
                         <div className="text-xs font-bold text-emerald-800 dark:text-emerald-300">متابعة ولي الأمر الفورية</div>
                         <div className="text-[11px] text-emerald-600 dark:text-emerald-400">إرسال تقرير الأداء والدرجات والغياب لولي الأمر</div>
                       </div>
                       <button
                         type="button"
                         onClick={() => handleOpenParentReport(editingStudent)}
                         className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all shadow cursor-pointer"
                       >
                         <Send className="w-3.5 h-3.5" />
                         إرسال واتساب
                       </button>
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                       <div>
                         ...
                         <input
                           ...
                           className="w-full px-3 py-2 border rounded-xl bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 text-sm font-mono outline-none"
                         />
                       </div>
                       ...
                       <div>
                         ...
                         <select
                           value={studentBehaviorRating}
                           onChange={(e) => setStudentBehaviorRating(e.target.value as any)}
                           className="w-full px-3 py-2 border rounded-xl bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 text-sm font-bold outline-none cursor-pointer"
                         >
                           <option value="excellent">🌟 متميز ومتفوق</option>
                           <option value="very_good">✨ مستوى طيب ومواظب</option>
                           <option value="good">⚡ متوسط يحتاج تدريب</option>
                           <option value="needs_followup">⚠️ يحتاج متابعة مكثفة</option>
                         </select>
                       </div>
                     </div>
                     ...
                     <div>
                       <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                         ملاحظات وتوجيهات مستر عبدالله لولي الأمر:
                       </label>
                       <textarea
                         rows={2}
                         value={studentTeacherNotes}
                         onChange={(e) => setStudentTeacherNotes(e.target.value)}
                         placeholder="توجيهات ونصائح مستر عبدالله لولي الأمر عن مستوى الطالب..."
                         className="w-full px-3 py-2 border rounded-xl bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 text-xs outline-none resize-none"
                       />
                       <div className="flex flex-wrap gap-1 mt-1.5">
                         <button
                           type="button"
                           onClick={() => setStudentTeacherNotes("طالب متميز جداً ومنضبط، ومستواه متقدم في إتقان قواعد اللغة وحل الأسئلة.")}
                           className="text-[10px] px-2 py-0.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 rounded-lg hover:bg-emerald-100 transition-colors"
                         >
                           + متفوق ومنضبط
                         </button>
                         <button
                           type="button"
                           onClick={() => setStudentTeacherNotes("مستوى طيب، نرجو حث الطالب على حل الكويزات الدورية وعدم تراكم الدروس.")}
                           className="text-[10px] px-2 py-0.5 bg-blue-50 text-blue-700 dark:bg-blue-950/30 rounded-lg hover:bg-blue-100 transition-colors"
                         >
                           + حث على الالتزام
                         </button>
                         <button
                           type="button"
                           onClick={() => setStudentTeacherNotes("الطالب بحاجة لإعادة مشاهدة حصص القواعد والتركيز في حل تدريبات المذكرة.")}
                           className="text-[10px] px-2 py-0.5 bg-amber-50 text-amber-700 dark:bg-amber-950/30 rounded-lg hover:bg-amber-100 transition-colors"
                         >
                           + يحتاج تدريب
                         </button>
                       </div>
                     ...
                     <div>
                       <label className="block text-xs font-bold text-slate-500 mb-1">تاريخ انتهاء الاشتراك المخصص:</label>
                       <input
                         type="date"
                         value={newSubExpiry}
        ...
        {/* TAB 7: Master Account & Auth Settings */}
        {tab === 'settings' && (
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-950 p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-md">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-1">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-700/50">
                    <ShieldAlert className="w-3.5 h-3.5 text-amber-500" />
                    <span>إدارة بيانات اعتماد المعلم والكنترول</span>
                  </div>
                  <h3 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2 pt-1">
                    إعدادات دخول المستر (مستر عبدالله سيد)
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    خصص البريد الإلكتروني وكلمة المرور الخاصة بك لتسجيل الدخول مباشرة دون أي خلط مع حسابات الطلاب.
                  </p>
                </div>

                {masterAuthUpdatedAt && (
                  <div className="text-xs text-slate-400 bg-slate-50 dark:bg-slate-900 px-3.5 py-2 rounded-xl border border-slate-100 dark:border-slate-800">
                    آخر تحديث: {new Date(masterAuthUpdatedAt).toLocaleDateString('ar-EG')} - {new Date(masterAuthUpdatedAt).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}
                  </div>
                )}
              </div>

              <form onSubmit={handleSaveMasterAuth} className="mt-6 max-w-2xl space-y-6">
                {/* Master Email Field */}
                <div>
                  <label className="block text-xs font-black text-slate-700 dark:text-slate-300 mb-2">
                    البريد الإلكتروني المخصص لتسجيل دخول المستر:
                  </label>
                  <div className="relative">
                    ...
                    <input
                      type="email"
                      required
                      value={masterAuthEmail}
                      onChange={(e) => setMasterAuthEmail(e.target.value)}
                      placeholder="master@mr-abdallah.com"
                      dir="ltr"
                      className="w-full pr-10 pl-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-amber-500 text-slate-900 dark:text-slate-100 outline-none text-left font-mono"
                    />
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                    هذا هو البريد الذي ستكتبه في خانة تسجيل الدخول العادية (دون أي تمييز خارجي) ليتم توجيهك فوراً وتلقائياً للوحة تحكم المستر.
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-xs font-black text-slate-700 dark:text-slate-300 mb-2">
                    كلمة مرور المستر المخصصة:
                  </label>
                  <div className="relative">
                    <Key className="absolute right-3 top-3 w-4 h-4 text-slate-400" />
                    <input
                      type={showMasterAuthPass ? 'text' : 'password'}
                      required
                      value={masterAuthPassword}
                      onChange={(e) => setMasterAuthPassword(e.target.value)}
                      placeholder="أدخل كلمة المرور الجديدة للمستر..."
                      className="w-full pr-10 pl-12 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-amber-500 text-slate-900 dark:text-slate-100 outline-none"
                    />
                    <button
                      type="button"
                      onClick={() => setShowMasterAuthPass(!showMasterAuthPass)}
                      className="absolute left-3 top-2.5 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer"
                    >
                      {showMasterAuthPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                    يمكنك كتابة أي كلمة مرور قوية ترغب بها، وسيتم التحقق منها عند الدخول.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="2026"
                      className="w-full pr-10 pl-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-amber-500 text-slate-900 dark:text-slate-100 outline-none font-mono"
                    />
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                    رمز الأمان يتيح لك الدخول السريع في أي وقت بضغطة زر واحدة حتى لو نسيت كلمة المرور الطويلة.
                  </p>
                </div>

                {/* Notification Alert Box */}
                <div className="p-4 bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-800/40 rounded-2xl flex items-start gap-3">
                  <ShieldCheck className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                  <div className="text-xs text-amber-900 dark:text-amber-200 space-y-1">
                    <p className="font-bold">فصل كامل ومحكم عن حسابات الطلاب:</p>
                    <p className="text-amber-800/80 dark:text-amber-300/80 leading-relaxed">
                      بيانات دخول المستر والإدارة مخزنة ومحمية بصورة مستقلة، ولا يمكن لأي طالب تسجيل الدخول بها أو الوصول إلى لوحة تحكم المستر.
                    </p>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isSavingMasterAuth}
                  className="px-8 py-3 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white font-black text-sm rounded-2xl shadow-lg transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  {isSavingMasterAuth ? (
                  ...
                  <span>حفظ وتحديث بيانات اعتماد المستر</span>
                </button>
              </form>
            </div>
          </div>
